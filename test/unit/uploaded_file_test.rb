...
class UploadedFileTest < ActiveSupport::TestCase
  ...
  should 'return a thumbnail for images' do
    f = UploadedFile.new
    f.expects(:image?).returns(true)
    f.expects(:full_filename).with(:thumb).returns(File.join(Rails.root, 'public', 'images', '0000', '0005', 'x.png'))
    assert_equal '/images/0000/0005/x.png', f.thumbnail_path
    f = UploadedFile.new
    f.stubs(:full_filename).with(:thumb).returns(File.join(Rails.root, 'public', 'images', '0000', '0005', 'x.png'))
