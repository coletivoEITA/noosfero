module Noosfero::Factory
  ...
  def create_user(name = nil, options = {}, person_options = {})
    ...
    fast_insert(Block, { :box_id => box.id, :type => 'MainBlock', :position => 0})
    ProfileBlock.create! :box_id => box.id, :position => 0
    #ArticleBlock.create! :box_id => box.id, :position => 0, :article_id => homepage.id
    user
  end
