...
module ApplicationHelper
  ...
  def profile_cat_icons(profile)
    if profile.is_a?(Enterprise)
      icons = profile.product_categories_names.map{ |c| c.size > 1 ? c[1] : nil }.compact.map do |c|
        cat_name = c.gsub( /[-_\s,.;'"]+/, '_' )
        cat_icon = "/images/icons-cat/#{cat_name}.png"
        if !File.exists? "#{RAILS_ROOT}/public/#{cat_icon}"
          cat_icon = '/images/icons-cat/undefined.png'
        end
        content_tag 'span', content_tag( 'span', c ), :title => c,
  ...
  def profile_image_link( profile, size=:portrait, tag='li', extra_info = nil, cat_icons = true )
    ...
    cat_icons = cat_icons ? profile_cat_icons(profile) : ''
    ...
    content_tag('div', content_tag(tag,
        ...
        link_to(
            ...
            extra_info + profile_sex_icon(profile) + cat_icons,
            profile.url,
