    ...
    module Taggable #:nodoc:
      ...
      module SingletonMethods
       ...
       def find_tagged_with(tags, options = {})
          tags = TagList.from(tags).names
          return [] if tags.empty?
          
          conditions = tags.map {|t| sanitize_sql(["tags.name LIKE ?",t])}.join(' OR ')
          conditions += ' AND ' + sanitize_sql(options.delete(:conditions)) if options[:conditions]
          group = "#{table_name}.id HAVING COUNT(#{table_name}.id) = #{tags.size}" if options.delete(:match_all)
          exclude = options.delete(:exclude)
          taggeds = find(:all, {:conditions => conditions, :include => :tags, :group => group}.update(options))
          exclude ? find(:all) - taggeds : taggeds
        end
        ...
        def save_tags
          ...
          self.class.transaction do
            ...
            new_tag_names.each do |new_tag_name|
              tags << (Tag.find(:first, :conditions => ['name like ?',new_tag_name]) || Tag.create(:name => new_tag_name))
            end
          end
          true
        end
